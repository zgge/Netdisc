			...
			<nav>
				  <ul class="pagination">
				  	<c:if test="${pageBean.currentPage!=1}">
						<li>
				      		<a href="${pageContext.request.contextPath}/listResourceByName?currentPage=${pageBean.currentPage-1} ">
				       		 <span>&laquo;</span>
				      		</a>
				    	</li>
					</c:if>
				    ...
				    <c:if test="${pageBean.totalPage<=6}">
						<c:forEach var="i" begin="1" end="${pageBean.totalPage}">
   							<c:if test="${pageBean.currentPage==i}">
   								<li><a href="${pageContext.request.contextPath}/listResourceByName?currentPage=${i} " class='B' style="background-color: #CCE8FF">${i}</a></li>
   							</c:if>
   							<c:if test="${pageBean.currentPage!=i}">
   								<li><a href="${pageContext.request.contextPath}/listResourceByName?currentPage=${i} " class='B'>${i}</a></li>
   							</c:if>
						</c:forEach>
					</c:if>
					<c:if test="${pageBean.totalPage>6}">
